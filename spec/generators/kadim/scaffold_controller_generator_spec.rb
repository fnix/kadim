...
RSpec.describe Kadim::Generators::ScaffoldControllerGenerator, type: :generator do
  ...
  context "with user defined attributes" do
    before do
      prepare_destination
      self.default_arguments = %w(User name birthdate:date)
      run_generator
    end

    it "uses the defined attributes on controller" do
      assert_file "app/controllers/kadim/users_controller.rb" do |content|
        assert_match(/def user_params/, content)
        assert_match(/params\.require\(:user\)\.permit\(:name, :birthdate\)/, content)
      end
    end
  end
